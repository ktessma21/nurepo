...
/**
  * @brief Edited from Random access memory (RAM) for nuPython
  *
  ...
  *
  * @note Prof. Joe Hummel and Kidus Tessma 
...
struct RAM_VALUE
{
  ...
  //
  // the actual value NULL when type != NONE:
  //
  struct object* obj_value;  // pointer to object
};

struct RAM_MAP
{
  char* hash_val;  // hash value of the object
